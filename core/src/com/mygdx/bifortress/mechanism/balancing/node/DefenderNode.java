...
public class DefenderNode extends Node{
    ...
    boolean shoot;
    float stateTime;
    public DefenderNode(int value, BinarySearchTree origin) {
        super(value, origin);
        headDeg = 0;
        initDeg = 0;
        maxTimeDelay = (10-level)/10*4;
    ...
    public void updateSelf(){
        if(this.lone == false){
            ...
            if(this.target != null){
                ...
                if(distance < 600f && Math.abs(initDeg-headDeg)<5){
    ...
    public void launchCanon(){
        ...
        CanonCell cell = new CanonCell(initX,initY,level*10,velocity*Math.cos(Math.toRadians(initDeg)),velocity*Math.sin(Math.toRadians(initDeg)), (int) level);
